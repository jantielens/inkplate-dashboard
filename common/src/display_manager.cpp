...
void DisplayManager::showVcomTestPattern() {
    Logger::begin("VCOM Test Pattern");
    ...
    if (!isnan(currentVcom)) {
        ...
        Logger::linef("Current VCOM: %.2f V", currentVcom);
    } else {
        vcomMessage += "N/A";
        Logger::line("Current VCOM: N/A (read failed)");
    }
    ...
    Logger::line("Displaying test pattern (1-bit checkerboard for Inkplate 2)");
    #else
    ...
    Logger::line("Displaying test pattern with 8 grayscale bars");
    #endif
